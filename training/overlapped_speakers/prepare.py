...
for speaker_path in glob.glob(os.path.join(DATASET_VIDEO_PATH, '*')):
    ...
    print(speaker_id)
    subprocess.check_output("mkdir {}".format(os.path.join(CURRENT_PATH, speaker_id, 'datasets', 'train')), shell=True)

    for s_path in glob.glob(os.path.join(DATASET_VIDEO_PATH, '*')):
        s_id = os.path.splitext(s_path)[0].split('\\')[-1]
        print(s_id)
        if s_path == speaker_path:
            subprocess.check_output("mkdir {}".format(os.path.join(CURRENT_PATH, speaker_id, 'datasets', 'train', s_id)), shell=True)
            subprocess.check_output("mkdir {}".format(os.path.join(CURRENT_PATH, speaker_id, 'datasets', 'val', s_id)), shell=True)
            n = 0
            for video_path in glob.glob(os.path.join(DATASET_VIDEO_PATH, speaker_id, '*')):
                video_id = os.path.splitext(video_path)[0].split('\\')[-1]
                print(video_id)
                if n < VAL_SAMPLES:
                    subprocess.check_output("mklink {}.mpg {}".format(os.path.join(CURRENT_PATH, speaker_id, 'datasets', 'val', s_id, video_id), video_path), shell=True)
                else:
                    subprocess.check_output("mklink {}.mpg {}".format(os.path.join(CURRENT_PATH, speaker_id, 'datasets', 'train', s_id, video_id), video_path), shell=True)
                n += 1
        else:
            subprocess.check_output("mklink {} {}".format(os.path.join(CURRENT_PATH, speaker_id, 'datasets', 'train', s_id), s_path), shell=True)
    subprocess.check_output("mklink {} {}".format(os.path.join(CURRENT_PATH, speaker_id, 'datasets', 'align'), DATASET_ALIGN_PATH), shell=True)